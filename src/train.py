"""Train a Pix2Pix model for semantic-label-map-to-real-image translation.

Run from the project root, for example:

    python -m src.train --data-root data/raw --output-dir outputs/runs/baseline --epochs 100
"""
...
from .models import build_pix2pix_models, set_requires_grad
from .utils import (
    append_csv_row,
    ensure_dir,
    get_device,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Pix2Pix for image-to-image translation.")
    ...
    parser.add_argument("--output-dir", type=str, default="outputs/runs/baseline", help="Directory for logs, figures and checkpoints.")
    ...
    parser.add_argument("--num-workers", type=int, default=0, help="Use 0 on Windows if multiprocessing causes issues.")
    ...
    parser.add_argument(
        ...
        default="vanilla",
        help="Adversarial loss: vanilla uses BCEWithLogitsLoss, lsgan uses MSE targets real=1/fake=0.",
    )
...
def load_resume_checkpoint(
    ...
    return int(checkpoint.get("epoch", 0)) + 1, float(checkpoint.get("best_val_mae", float("inf")))
...
def train_one_epoch(
    ...
    progress = tqdm(dataloader, desc=f"Epoch {epoch:03d} [train]", leave=False)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if args.sample_every > 0 and (epoch == 1 or epoch % args.sample_every == 0):
            save_epoch_samples(
                ...
                title=f"Validation samples - epoch {epoch}",
            )
